...
exports.AddNewContact = (req, res) => {
	...
	ContactsModel.create(req.body, (err, result) => {
...
exports.GetContactById = (req, res) => {
	ContactsModel.findOne({ _id: Types.ObjectId(req.params.id), addedBy: req.user._id })
		.lean()
		.exec((err, contact) => {
...
exports.UpdateContactById = (req, res) => {
	ContactsModel.findByIdAndUpdate(
		req.params.id,
		{ $set: req.body },
		{ new: true },
		(err, result) => {
...
exports.DeleteContactById = (req, res) => {
	ContactsModel.findByIdAndRemove(req.params.id, err => {
		if (err) return res.status(500).send({ message: err.message });
